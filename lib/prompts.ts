...
const RUBRIC_DEFINITIONS = `RUBRIC (score each 0–20):
- rapport (Rapport & listening): Did the provider acknowledge the patient's concerns, ask questions, and respond to what the patient actually said — or steamroll through a script? Interrupting, ignoring stated worries, or generic empathy phrases score low. Reflecting the patient's own words and situation scores high.
- framing (Clinical framing): Did the provider tie the recommendation to THIS patient's exam findings and the consequences of inaction — not generic benefits? Citing the specific findings from the chart (measurements, failed treatments, exam results) and what happens if untreated scores high. "This works great for lots of patients" scores low.
- price (Price delivery): Did the provider state the price plainly and confidently — no apologizing ("I know it's a lot..."), no unprompted discounting, no burying it in a word salad — and tolerate silence after saying it? An unprompted discount caps this category at 8/20. Never stating a price at all caps it at 5/20.
- objections (Objection handling): When the patient objected, did the provider isolate the REAL objection with a question ("If the cost weren't a factor, would you want to do this?") rather than retreating, over-explaining, or capitulating? Caving immediately or arguing scores low; curious isolation followed by a targeted answer scores high.
- close (Asking for the close): Did the provider explicitly ask for a decision? An assumptive or alternative close ("Let's get your first session scheduled — do mornings or afternoons work?") scores highest (16–20). A direct question ("Would you like to move forward?") scores well (11–15). Hinting without asking scores 6–10. Never asking for the close at all caps this category at 5/20.
